...
Vector page_rank_Jacobi(string test_path, double p){
    ...
    Vector xo = Vector::Zero(n); 
    Vector b = Vector::Ones(n);

    Vector ranks = jacobi(xo, b, A, 10000, 1e-5); 
...
Vector page_rank_GS(string test_path, double p){
    ...
    Vector ranks = gauss_seidel(xo, b, A, 10000, 1e-5);
...
void correr_test_catedra(){
    ...
    for (string test : tests) {
        ...
        Vector puntajes_finales = page_rank_GS(test, p[i-1]); 
...
void testBasico() {
    ...
    Vector jac = gauss_seidel(x, b, sm, 1000, 1e-10);
